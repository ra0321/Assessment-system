...
const NavbarVertical = (props) => {
	...
	// Force redirect for unsigned user
			...
			<SimpleBar style={{ maxHeight: '100vh' }}>
				...
				<Accordion defaultActiveKey="0" as="ul" className="navbar-nav flex-column">
					{DashboardMenu.map(function (menu, index) {
						if (menu.grouptitle) {
							...
							if (menu.children) {
								...
								if (localStorage.getItem('isAdmin') == 'true' || (localStorage.getItem('isAdmin') != 'true' && !menu.isAdmin)) {
									return (
										<Card bsPrefix="nav-item" key={index}>
					...
					<Card bsPrefix="nav-item" >
						{/* Logout */}
						<Link href="#" onClick = {onLogout} className={`nav-link`}>
							<i className={`nav-icon fe fe-external-link me-2`}></i>
							Logout 
						</Link>
